...
def process_images(df, name):
    ...
    feature_df["embedding"].astype(np.ndarray)

    for idx, img_path in df["file_path"].items():
        if img_path.lower().endswith(('.png', '.jpg', '.jpeg')):
            img_array = load_and_process_image(img_path)
            feature_df.loc[idx, "embedding"] = model.predict(img_array)
            feature_df.to_csv("features_" + name + ".csv")


process_images(train_df[:10], "train")
process_images(val_df[:10], "val")
process_images(test_df[:10], "test")
